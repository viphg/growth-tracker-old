# PostgreSQL Installation Diagnosis Script
# PowerShell version

Write-Host "===========================================" -ForegroundColor Cyan
Write-Host "PostgreSQL Installation Diagnosis Script" -ForegroundColor Cyan
Write-Host "===========================================" -ForegroundColor Cyan

Write-Host "`n1. Checking PostgreSQL installation path..." -ForegroundColor Yellow
...
if (-not $foundPg) {
    ...
    if ($pgFound) {
        ...
        # Try to run directly in possible paths
        foreach ($pathPattern in $pgPaths) {
            $paths = Get-Item $pathPattern -ErrorAction SilentlyContinue | Sort-Object Name -Descending
            foreach ($path in $paths) {
                $binPath = Join-Path $path "bin"
